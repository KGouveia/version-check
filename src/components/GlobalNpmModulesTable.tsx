...
interface GlobalNpmModulesTableProps {
...
export const GlobalNpmModulesTable = ({
  modules,
  isBusy,
  upgradingPackage,
  onOpenNpm,
  ...
  if (modules.length === 0) {
    return (
      <div className="px-6 py-12 text-center">
        <p className="text-sm text-zinc-400">No global npm packages were found.</p>
      </div>
    );
  }
                ...
                <td className="px-2 py-2">
                  <div className="flex justify-end gap-1">
                    ...
                    <button
                      ...
                      disabled={isBusy || !upgradeEnabled}
                      title={
                        upgradeEnabled && upgradeSpec
                          ? `Upgrade ${item.name} (npm install -g ${item.name}@${upgradeSpec})`
